package com.addplus.server.web.oss.service.impl;

import com.addplus.server.api.constant.ErrorCode;
import com.addplus.server.api.model.oss.Policy;
import com.addplus.server.api.model.oss.ReturnPolicy;
import com.addplus.server.api.utils.date.DateUtils;
import com.addplus.server.web.oss.OssClientUtils;
import com.addplus.server.web.oss.config.OssConfig;
import com.addplus.server.web.oss.service.OssService;
import com.addplus.server.web.oss.uploader.FileUploader;
import com.addplus.server.web.oss.utils.OssFileUtils;
...
public class OssServiceImpl implements OssService {
    ...
    public ReturnPolicy createPolicy(String fileName, Boolean randomName) {
        ...
        // 获取文件名
        String key = OssFileUtils.getUploadPath(fileName, randomName);
        String dir = OssFileUtils.getFileType().get(OssFileUtils.getSuffix(fileName));
        ...
        try {
            ...
            returnPolicy.setErrorInfo(ErrorCode.SYS_SUCCESS.getError());
            returnPolicy.setReturnCode(ErrorCode.SYS_SUCCESS.getCode());
            return returnPolicy;
            ...
            try {
                ...
                for (MultipartFile file : files) {
                    if (!file.isEmpty()) {
                        ...
                        String contentType = OssFileUtils.getFileContentType(file.getOriginalFilename());
                        objectMetadata.setContentType(contentType);
                        String key = OssFileUtils.getUploadPath(file.getOriginalFilename(), true);
